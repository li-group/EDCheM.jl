...
function modgen(n_loc,Location,Location_tr,trline,Param,plan_max,n_lij,n_bun_agg)
	...
	function transcon2(m) #Function to add constraints on power flow in transmision lines
	    ...
	    @constraint(m,tr3[(p,v) in trline,t=1:n_tm,k=1:n_k,h=1:n_s,l=1:n_lij],-0.1*Fl_max[(p,v)]*n[(p,v),l].<=0.1*p_flow[(p,v),l,t,k,h]*S_base)
	    @constraint(m,tr4[(p,v) in trline,t=1:n_tm,k=1:n_k,h=1:n_s,l=1:n_lij],0.1*p_flow[(p,v),l,t,k,h]*S_base.<=0.1*Fl_max[(p,v)]*n[(p,v),l])
